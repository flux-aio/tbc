...
local function recently_charged_same_target(now)
    -- Signal 1: addon-triggered Charge — check GUID match
    if (now - last_charge_time) < CHARGE_INTERCEPT_COOLDOWN then
        local target_guid = UnitGUID(TARGET_UNIT)
        if target_guid and target_guid == last_charge_guid then return true end
    end
    -- Signal 2: Charge just went on CD (catches manual charges) — no GUID info, blanket suppress
