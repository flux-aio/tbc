...
rotation_registry:register("shadow", {
    ...
    named("EnsureShadowform", {
        matches = function(context, state)
            if context.in_shadowform then return false end
    ...
    named("VampiricEmbrace", {
        matches = function(context, state)
            ...
            if not context.settings.shadow_ve_maintain then return false end
            return state.ve_remaining < 3
        end,
        execute = function(icon, context, state)
    ...
    named("DevouringPlague", {
        matches = function(context, state)
            if not context.in_combat then return false end
            if not context.has_valid_enemy_target then return false end
            if not context.settings.shadow_use_devouring_plague then return false end
    ...
    named("InnerFocus", {
        ...
        matches = function(context, state)
            if not context.in_combat then return false end
            if not context.settings.shadow_use_inner_focus then return false end
    ...
    named("ShadowWordDeath", {
        matches = function(context, state)
            if not context.in_combat then return false end
            if not context.has_valid_enemy_target then return false end
            if not context.settings.shadow_use_swd then return false end
